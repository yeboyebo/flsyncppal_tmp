...
class OrderSerializer(DefaultSerializer):
    ...
    def get_data(self):
        ...
        self.set_string_value("codalmacen", qsatype.FactoriaModulos.get('flfacturac').iface.pub_valorDefecto("codalmamayor"))
        self.set_string_value("observaciones", "Pedido B2B: " + self.init_data["increment_id"] + "\nTeléfono: " + self.init_data["shipping_address"]["telephone"])
        ...
        existe_gasto_envio = parseFloat(self.init_data["shipping_price"])
        if existe_gasto_envio > 0:
            for item in self.init_data["shipping_price"]:
                item.update({
                    "codigo_cliente": codigo_cliente,
                    "codigo_serie": codigo_serie
                })
                linea_envio = OrderShippingLineSerializer().serialize(item)
